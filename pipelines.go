...
func (c *Client) GetPipeline(app, pipeline string) (*Pipeline, error) {
	var p Pipeline
	if err := c.GetWithRetry(c.URLs["gate"] + "/applications/" + app + "/pipelineConfigs/" + pipeline, &p); err != nil {
		return nil, err
	}
	return &p, nil
}
...
func (c *Client) GetPipelines(app string) ([]Pipeline, error) {
	...
	if err := c.GetWithRetry(c.URLs["gate"] + "/applications/" + app + "/pipelineConfigs", &pipelines); err != nil {
